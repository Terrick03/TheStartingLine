...
boolean player1answerSent = false;
boolean player2answerSent = false;
...
void checkButtons() {
  ...
  for (int i = 0; i < numberOfAnswers; i++) {
    ...
    if (analogRead(pinReading1[i]) > 1000) {
      //Serial.print("PRESSED");
      //Serial.println(i);
      if (!player1answerSent) {
        if (i == 0) {
          Serial.println(0);
          player1answerSent = true;
        }
        if (i == 1) {
          Serial.println(1);
          player1answerSent = true;
      ...
      if (!player2answerSent) {
        if (i == 2) {
          Serial.println(2);
          player2answerSent = true;
        }
        if (i == 3) {
          Serial.println(3);
          player2answerSent = true;
  ...
  if (digitalRead(pinReading) == HIGH && !moderatorButton) {
    ...
    answers[1] = false;
    answers[0] = false;
    player1answerSent = false;
    player2answerSent = false;
    // delay (120);
